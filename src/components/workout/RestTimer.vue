...
<template>
  ...
  >
    ...
    >
      ...
      <span class="flex-1 text-center font-mono text-2xl font-semibold tabular-nums">{{ label }}</span>
      <Button variant="outline" size="sm" aria-label="Añadir 30 segundos" @click="emit('add', 30)">
        <Plus class="size-4" />
        30s
      </Button>
      <Button variant="ghost" size="icon" aria-label="Saltar descanso" @click="emit('stop')">
        <X class="size-4" />
      </Button>
    </div>
  </Transition>
